...
def build_readiness_report(*, settings: Settings) -> ReadinessReport:
    ...
    if (
        settings.llm.required_provider is not None
        and settings.llm.required_provider != settings.llm.provider
    ):
        checks.append(
            _check(
                "required_provider",
        ...
        checks.append(
            _check(
                ...
                detail=(
                    ...
                    if shared_rate_limiting_required
                    else "shared rate limiting not required"
                ),
            )
        )
