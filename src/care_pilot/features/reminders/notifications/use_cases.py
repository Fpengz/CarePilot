...
def _message_for_event(event: WorkflowTimelineEvent) -> str:
    ...
    if workflow_name == "alert_only":
        ...
        return (
            "Alert delivery workflow completed successfully"
            if success
            else "Alert workflow completed with issues"
        )
...
def _notification_from_event(
    *,
    event: WorkflowTimelineEvent,
    reads: NotificationReadStateStore,
    user_id: str,
) -> NotificationItem:
...
def mark_all_notifications_read(
    *, context: "AppContext", user_id: str
    ...
    return NotificationMarkAllReadResponse(
        updated_count=updated_count, unread_count=refreshed.unread_count
    )
