"""Daily nutrition summary calculations for meal history review.
...
def build_daily_nutrition_summary(
    *,
    ...
    day_records = [
        ...
        if local_date_for(record.captured_at, timezone_name=timezone_name)
        == summary_date
    ]
    ...
    return DailyNutritionSummary(
        ...
        last_logged_at=(
            day_records[-1].captured_at.isoformat() if day_records else None
        ),
        consumed=consumed,
        targets=targets,
        remaining=remaining,
...
def _build_insights(
    *,
    ...
    window_records = [
        ...
        for record in meal_history
        if window_start
        <= local_date_for(record.captured_at, timezone_name=timezone_name)
        <= summary_date
    ]
    distinct_days = {
        local_date_for(
    ...
    average_protein = sum(
        meal_nutrition(record).protein_g for record in window_records
    ) / len(window_records)
    protein_dense_share = sum(
        1
        for record in window_records
        if meal_nutrition(record).protein_g >= 20.0
    ) / len(window_records)
    if average_protein < 18.0 or protein_dense_share < 0.4:
        insights.append(
            NutritionInsight(
                ...
                actions=[
                    "Choose meals with tofu, eggs, fish, or chicken.",
                    "Pair carb-heavy dishes with a higher-protein side.",
                ],
            )
        )

    average_fiber = sum(
        float(meal_nutrition(record).fiber_g or 0.0)
        for record in window_records
    ) / len(window_records)
    if average_fiber < 5.0:
        insights.append(
            NutritionInsight(
                ...
                actions=[
                    "Look for higher-fiber options such as vegetables, beans, or whole grains.",
                    "Add fruit or greens to one meal today.",
                ],
            )
        )
    ...
    avg_daily_sodium = sum(
        bucket.sodium_mg for bucket in daily_totals.values()
    ) / len(daily_totals)
    sodium_heavy_meals = sum(
        1
        for record in window_records
        if meal_nutrition(record).sodium_mg > 900.0
    )
    if (
        avg_daily_sodium > float(profile.daily_sodium_limit_mg) * 0.9
        or sodium_heavy_meals >= 3
    ):
        insights.append(
            NutritionInsight(
                ...
                actions=[
                    "Prefer soups or sauces with less gravy.",
                    "Swap one salty dish for a lower-sodium option today.",
                ],
            )
        )

    avg_daily_sugar = sum(
        bucket.sugar_g for bucket in daily_totals.values()
    ) / len(daily_totals)
    sugar_heavy_meals = sum(
        1 for record in window_records if meal_nutrition(record).sugar_g > 15.0
    )
    if (
        avg_daily_sugar > float(profile.daily_sugar_limit_g) * 0.9
        or sugar_heavy_meals >= 3
    ):
        insights.append(
            NutritionInsight(
                ...
                actions=[
                    "Choose drinks and sauces with less added sugar.",
                    "Favor lower-sugar meals for your next meal slot.",
                ],
            )
        )
    ...
    if calorie_target > 0:
        avg_daily_calories = sum(
            bucket.calories for bucket in daily_totals.values()
        ) / len(daily_totals)
        if avg_daily_calories > calorie_target * 1.1:
            insights.append(
                NutritionInsight(
                    ...
                    actions=[
                        "Use a smaller portion size for one meal today.",
                        "Balance rich meals with a lighter next meal.",
                    ],
                )
            )

    repeats = Counter(
        _normalize_dish_name(meal_display_name(record))
        for record in window_records
    )
    if any(count >= 3 for count in repeats.values()):
        insights.append(
            NutritionInsight(
                ...
                actions=[
                    "Try one new protein or vegetable source this week.",
                    "Rotate between two or three meal types instead of repeating one dish.",
                ],
            )
        )
