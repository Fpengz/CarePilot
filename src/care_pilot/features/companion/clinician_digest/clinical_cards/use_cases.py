...
def _resolve_date_window(
    ...
    if start_date > end_date:
        raise build_api_error(
            status_code=400,
            code="clinical_cards.invalid_window",
...
def generate_clinical_card_for_session(
    *,
    ...
    ldl_trend = build_metric_trend(
        "biomarker:ldl",
        biomarker_points(biomarker_readings, biomarker_name="ldl"),
    )
    hba1c_trend = build_metric_trend(
        "biomarker:hba1c",
...
def get_clinical_card_for_session(
    *,
    ...
    if item is None:
        raise build_api_error(
            status_code=404,
            code="clinical_cards.not_found",
            message="clinical card not found",
        )
