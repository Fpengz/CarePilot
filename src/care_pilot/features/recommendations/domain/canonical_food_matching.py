...
def _risk_tags(
    *,
    ...
    if (
        ...
        and nutrition.fiber_g is not None
        and nutrition.fiber_g < 5
        and "refined_carb" not in tags
    ):
    ...
    if (
        ...
        and (nutrition.fiber_g or 0.0) >= 5
...
def _from_teammate_entry(
    entry: dict[str, object], *, base: CanonicalFoodRecord | None = None
) -> CanonicalFoodRecord:
    ...
    calories = _number(
        ...
        base.nutrition.calories if base is not None else 0,
    )
    derived_price_tier = cast(
        ...
        (
            base.price_tier
            if base is not None
            else _price_tier(str(entry.get("source") or ""), calories)
        ),
    )
    ...
    merged_health_tags = list(
        dict.fromkeys(
            [
                *(base.health_tags if base is not None else []),
                *[str(item) for item in health_tags_raw],
            ]
        )
    )
    preparation_tags = list(
        dict.fromkeys(
            [
                *_preparation_tags(title, str(entry.get("source") or "")),
                *(base.preparation_tags if base is not None else []),
            ]
        )
    )
    merged_nutrition = Nutrition(
        ...
        carbs_g=_number(
            ...
            base.nutrition.carbs_g if base is not None else 0,
        ),
        sugar_g=_number(
            nutrition.get("sugar_g"),
            base.nutrition.sugar_g if base is not None else 0,
        ),
        protein_g=_number(
            nutrition.get("protein_g"),
            base.nutrition.protein_g if base is not None else 0,
        ),
        fat_g=_number(
            nutrition.get("total_fat_g"),
            base.nutrition.fat_g if base is not None else 0,
        ),
        sodium_mg=_number(
            nutrition.get("sodium_mg"),
            base.nutrition.sodium_mg if base is not None else 0,
        ),
        fiber_g=_number(
            nutrition.get("fiber_g"),
            (
                base.nutrition.fiber_g
                if base is not None and base.nutrition.fiber_g is not None
                else 0
            ),
        ),
    )
    return CanonicalFoodRecord(
        ...
        cuisine_tags=list(
            dict.fromkeys(
                [
                    str(entry.get("cuisine") or "").strip(),
                    *(base.cuisine_tags if base is not None else []),
                ]
            )
        ),
...
def build_default_canonical_food_records() -> list[CanonicalFoodRecord]:
    ...
    """
    from care_pilot.platform.persistence.food.ingestion import (  # noqa: PLC0415
