...
LOCAL_DISH_ALTERNATIVES = {
    "laksa": [
        "Try fish soup with brown rice",
        "Ask for less gravy and no extra sambal",
    ],
    "mee rebus": [
        "Request half noodles and extra bean sprouts",
        "Skip sweet gravy top-up",
    ],
    "char kway teow": [
        "Choose yong tau foo soup",
        "Avoid added lard and sausage",
    ],
}


def _local_advice_for_dish(
    dish_name: str,
    ...
    if user_profile.preferred_cuisines:
        base.append(
            f"Bias future swaps toward your preferred cuisines: {', '.join(user_profile.preferred_cuisines)}."
        )
    if user_profile.disliked_ingredients:
        base.append(
            f"Avoiding your stated dislikes: {', '.join(user_profile.disliked_ingredients)}."
        )
    return base
