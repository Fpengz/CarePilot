...
def _build_meal_event(candidate: FoodItem) -> MealEvent:
    return MealEvent(
        ...
        ingredients=[
            Ingredient(name=name) for name in candidate.ingredient_tags
        ],
...
def _score_similarity(
    candidate: FoodItem, record: MealRecognitionRecord | FoodItem
) -> float:
    if isinstance(record, FoodItem):
        cuisine_score = _overlap_score(
            set(candidate.cuisine_tags), set(record.cuisine_tags)
        )
        ...
        venue_score = 0.0
        slot_score = (
            1.0 if candidate.slot == _infer_slot(record.captured_at) else 0.0
        )
        title_score = _overlap_score(
            set(normalize_text(candidate.title).split()),
...
def _matches_restriction(
    candidate: FoodItem, restricted_terms: set[str]
) -> bool:
    searchable = " ".join(
        [
            candidate.title,
...
def _slot_baseline(
    ...
    items = [
        meal_nutrition(record)
        for record in meal_history
        if _infer_slot(record.captured_at) == slot
    ]
    if not items:
...
def _candidate_health_delta(
    ...
    *,
    ...
    baseline_calories, baseline_sugar, baseline_sodium = _slot_baseline(
        slot, meal_history
    )
    return HealthDelta(
...
def _preference_fit(
    snapshot: PreferenceSnapshot, candidate: FoodItem
) -> float:
...
def _temporal_fit(
    ...
    *,
    ...
    if normalized_title in {
        normalize_text(item) for item in temporal.recent_repeat_titles
    }:
        score -= 0.35
...
def _health_gain(
    ...
    *,
    ...
    delta = _candidate_health_delta(
        candidate, slot=slot, meal_history=meal_history
    )
    reasons: list[str] = []
    score = 0.0
    ...
    if delta.sugar_g < 0:
        score += min(
            abs(delta.sugar_g) / max(profile.daily_sugar_limit_g, 1.0), 0.3
        )
    ...
    if snapshot is not None:
        ...
        if (
            biomarkers.get("hba1c", 0.0) >= 7.0
            and "lower_sugar" in candidate.health_tags
        ):
            score += 0.15
        if (
            biomarkers.get("ldl", 0.0) >= 3.4
            and "heart_health" in candidate.health_tags
        ):
            score += 0.1
        if (
            biomarkers.get("systolic_bp", 0.0) >= 140
            or biomarkers.get("diastolic_bp", 0.0) >= 90
...
def _adherence_likelihood(
    ...
    if snapshot.interaction_count <= 0:
        ...
        behavior_signal = snapshot.accepted_count / max(
...
def _candidate_constraints(
    ...
    *,
    ...
    if candidate.nutrition.sugar_g > profile.daily_sugar_limit_g:
        ...
        return (False, constraints, caution_notes)
    try:
        safety_warnings = SafetyEngine(profile).validate_meal(
            _build_meal_event(candidate)
        )
