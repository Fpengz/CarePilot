...
def _build_workflow_with_timeline(
    *, request_id: str, correlation_id: str, events: list[dict[str, object]]
) -> dict[str, object]:
    return {
    ...
    *,
    ...
    if safety["decision"] != "allow":
        ...
        payload: dict[str, Any] = {
            ...
            "report_parse": {
                "readings": [],
                "snapshot": {"biomarkers": {}, "risk_flags": []},
            },
            "recommendation": {
                "safe": False,
...
def list_suggestions_for_session(
    *,
    ...
    source_user_ids, source_display_names = _source_scope(
        scope=scope, session=session, household_store=household_store
    )
