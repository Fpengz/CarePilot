...
SINGAPORE_CANDIDATES: tuple[MealCandidate, ...] = (
    ...
    MealCandidate(
        ...
        ingredients=("lentils",),
        caution_notes=(
            "Keep chutney portions modest if sodium intake is a concern.",
        ),
    ),
    MealCandidate(
        slot="lunch",
        ...
        caution_notes=(
            "Ask for less soup if you need tighter sodium control.",
        ),
    ),
    MealCandidate(
        slot="lunch",
    ...
    MealCandidate(
        ...
        caution_notes=(
            "Skip fried items and choose fewer fish-ball style pieces.",
        ),
    ),
    MealCandidate(
        slot="dinner",
        ...
        caution_notes=(
            "Request less rice and sauce for tighter glucose control.",
        ),
    ),
    MealCandidate(
        slot="snack",
...
def _latest_snapshot_from_history(
    readings: Sequence[BiomarkerReading],
...
def _contains_restricted_term(
    candidate: MealCandidate, blocked_terms: set[str]
) -> bool:
    searchable = " ".join(
        (candidate.title, " ".join(candidate.ingredients))
    ).lower()
...
def _score_candidate(
    ...
    *,
    ...
    cuisines = set(
        cuisine.lower() for cuisine in health_profile.preferred_cuisines
    )
    ...
    if matched_goals:
        score += 0.18
        why.append(
            f"Matches your nutrition goals: {', '.join(matched_goals)}."
        )
    ...
    if matched_cuisines:
        score += 0.12
        why.append(
            f"Fits your preferred cuisine: {', '.join(matched_cuisines)}."
        )
    ...
    if snapshot is not None:
        ...
        if biomarkers.get("hba1c", 0) >= 7.0 and "lower_sugar" in traits:
            score += 0.1
            why.append(
                "Favors steadier glucose control based on recent HbA1c."
            )
        if biomarkers.get("ldl", 0) >= 3.4 and "heart_health" in traits:
            score += 0.1
            why.append(
                "Supports lower saturated-fat choices based on recent LDL."
            )
        if (
            biomarkers.get("systolic_bp", 0) >= 140
            or biomarkers.get("diastolic_bp", 0) >= 90
        ):
            if "lower_sodium" in traits:
                score += 0.1
                why.append(
                    "Leans lower-sodium to match elevated blood-pressure readings."
                )
    ...
    if (
        last_meal_name
        and last_meal_name.lower() not in candidate.title.lower()
    ):
        score += 0.03
    ...
    if not why:
        why.append(
            "Uses a generally lighter local option while your profile is still incomplete."
        )
    ...
    *,
    ...
    blocked_terms = {
        ...
        for item in [
            ...
            *health_profile.disliked_ingredients,
        ]
        if item.strip()
    }
    ...
    if fallback_mode:
        warnings.append(
            "Personalization is running in fallback mode until more health profile data is completed."
        )
    if snapshot is None:
        warnings.append(
            "No recent biomarker snapshot is available, so suggestions use profile-only heuristics."
        )
    ...
    for slot in ("breakfast", "lunch", "dinner"):
        candidates = [
            candidate
            for candidate in SINGAPORE_CANDIDATES
        ...
        if not ranked:
            warnings.append(
                f"No clean {slot} candidate matched the current restrictions; broaden profile preferences."
            )
            continue
    ...
    snack_candidate = next(
        (
            candidate
            for candidate in SINGAPORE_CANDIDATES
            if candidate.slot == "snack"
        ),
        None,
    )
    if snack_candidate is not None and not _contains_restricted_term(
        snack_candidate, blocked_terms
    ):
        score, why = _score_candidate(
            snack_candidate,
            profile=user_profile,
