"""Biomarker parsing domain logic.

Provides pattern-based extraction of biomarker values from free-text or PDF
reports and pure rule evaluation to flag clinical risk conditions.

No I/O, no persistence — callers supply the ``ReportInput`` and receive
...
def parse_report_input(report_input: ReportInput) -> list[BiomarkerReading]:
    """Extract biomarker readings from a ``ReportInput``."""
    ...
    logger.info(
        "parse_report_input_complete source=%s readings=%s",
        report_input.source,
        len(readings),
    )
    return readings
...
def build_clinical_snapshot(
    ...
    """Derive a ``ClinicalProfileSnapshot`` with risk flags from biomarker readings."""
