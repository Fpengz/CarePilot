    ...
    def get_model(
        ...
        *,
        ...
        if runtime.provider == ModelProvider.TEST.value:
            logger.info(
                "provider_selected provider=test model=%s capability=%s",
                runtime.model_name,
        ...
        if runtime.provider == ModelProvider.GEMINI.value:
            ...
            logger.info(
                "provider_selected provider=gemini model=%s capability=%s",
                runtime.model_name,
                runtime.capability or "none",
            )
        ...
        if runtime.provider == ModelProvider.OPENAI.value:
            ...
            if not api_key:
                ...
                return LLMFactory._attach_model_name(TestModel(), "test-model")
            provider_obj = LLMFactory._build_openai_provider(
                api_key=api_key, base_url=runtime.base_url
            )
