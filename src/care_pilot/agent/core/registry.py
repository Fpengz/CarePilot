...
class AgentRegistry:
    def __init__(
        ...
        *,
        ...
        self._workflow_contracts = {
            contract.workflow_name: contract for contract in workflow_contracts
        }
...
def build_default_agent_registry() -> AgentRegistry:
    agents = [
        AgentContract(
            agent_id="meal_analysis_agent",
            capabilities=["meal_perception", "meal_normalization"],
            allowed_tools=[],
            output_contract="vision_result_with_enriched_event",
        ),
        AgentContract(
            agent_id="dietary_agent",
            capabilities=["nutrition_risk_assessment", "dietary_reasoning"],
            allowed_tools=[],
            output_contract="dietary_reasoning_response",
        ),
        AgentContract(
            agent_id="recommendation_agent",
            capabilities=["daily_recommendation", "substitution_ranking"],
            allowed_tools=[],
            output_contract="daily_recommendation_bundle",
        ),
        AgentContract(
            agent_id="emotion_agent",
            capabilities=["emotion_inference"],
            allowed_tools=[],
            output_contract="emotion_inference_result",
        ),
        AgentContract(
            agent_id="chat_agent",
            capabilities=["chat_response", "chat_stream"],
            allowed_tools=[],
            output_contract="chat_response",
        ),
        AgentContract(
    ...
    contracts = [
        WorkflowRuntimeContract(
            ...
            steps=[
                WorkflowRuntimeStep(
                    step_id="meal_analysis",
                    agent_id="meal_analysis_agent",
                    capability="meal_perception",
                    tool_names=[],
                ),
                WorkflowRuntimeStep(
                    step_id="dietary_reasoning",
                    agent_id="dietary_agent",
        ...
        WorkflowRuntimeContract(
            ...
            steps=[
                WorkflowRuntimeStep(
                    ...
                    agent_id="meal_analysis_agent",
                    capability="meal_normalization",
                    tool_names=[],
                ),
                WorkflowRuntimeStep(
                    step_id="summarize",
                    agent_id="dietary_agent",
                    capability="dietary_reasoning",
                    tool_names=[],
                ),
            ],
        ),
