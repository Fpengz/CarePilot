...
``ToolRegistry`` is the runtime container for registered agent tools.  It
stores ``ToolSpec`` definitions alongside callable handlers, enforces scope and
environment policy guards, validates input/output schemas, and records
...
class ToolRegistry:
    """Runtime registry that validates, dispatches, and meters tool calls."""
