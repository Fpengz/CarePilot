...
from dietary_guardian.domain.notifications.models import ReminderNotificationChannel
...
class OutboxWorker:
    ...
    async def _deliver_record(self, record: OutboxRecord) -> AlertDeliveryResult:
        ...
        if sink is None:
            self._sync_reminder_notification_dead_letter(record=record, attempt=record.attempt_count + 1, error="unknown sink")
    ...
    def _sync_reminder_notification_processing(self, *, record: OutboxRecord, attempt: int) -> None:
        ...
        if callable(append_log):
            ...
            from dietary_guardian.domain.notifications.models import ReminderNotificationLogEntry
