...
from dietary_guardian.features.reminders.domain.models import MedicationRegimen, ReminderEvent
...
def _latest_seeded_day(db_path: str, user_id: str) -> date | None:
    queries = [
        ("SELECT MAX(captured_at) FROM meal_nutrition_risk_profiles WHERE user_id = ?", user_id),
        ("SELECT MAX(scheduled_at) FROM medication_adherence_events WHERE user_id = ?", user_id),
        ("SELECT MAX(scheduled_at) FROM reminder_events WHERE user_id = ?", user_id),
        ("SELECT MAX(measured_at) FROM biomarker_readings WHERE user_id = ?", user_id),
    ]
...
def _resolve_window(
    *,
    ...
    if start_date is not None:
        ...
        start = (latest + timedelta(days=1)) if latest is not None else (datetime.now(UTC).date() - timedelta(days=days - 1))
    else:
...
def _slot_blueprints(profile: SyntheticProfile) -> list[tuple[str, int, int, int]]:
    if profile == "volatile":
        return [
            ("breakfast", 8, 480, 18),
            ("lunch", 13, 760, 32),
...
def _build_profile(*, user_id: str, config: SyntheticProfileConfig) -> HealthProfileRecord:
    return HealthProfileRecord(
        ...
        conditions=[MedicalCondition(name="Type 2 Diabetes", severity="High"), MedicalCondition(name="High Cholesterol", severity="Medium")],
        medications=[Medication(name="Metformin", dosage="500mg"), Medication(name="Atorvastatin", dosage="20mg")],
...
def seed_synthetic_data(
    *,
    ...
    start, end = _resolve_window(db_path=db_path, user_id=user_id, days=days, start_date=start_date, append=append)
    ...
    for offset in range(days):
        ...
        for slot_index, (slot, hour, base_calories, base_carbs) in enumerate(slot_blueprints[:daily_meal_count]):
            ...
            calories = max(140.0, base_calories + day_variation + trend_adjustment + rng.uniform(-50.0, 50.0))
            ...
            risk_tags: list[str] = []
            if sugar >= 18 or (profile == "improving" and progress < 0.45 and slot in {"breakfast", "snack"}):
            ...
            repo.save_validated_meal_event(
                ValidatedMealEvent(
                    ...
                    provenance={"source": "synthetic_seed", "profile": profile, "slot": slot},
                )
            )
        ...
        morning_status: Literal["taken", "missed"] = (
            "taken" if rng.random() > (0.24 if profile == "volatile" else 0.12 if profile == "stable" else 0.18 * (1.0 - progress)) else "missed"
        )
        ...
        for regimen, scheduled_at, status in adherence_specs:
            ...
            repo.save_medication_adherence_event(
                MedicationAdherenceEvent(
                    ...
                    status=status,
                    scheduled_at=scheduled_at,
                    taken_at=scheduled_at + timedelta(minutes=5 + rng.randint(0, 20)) if status == "taken" else None,
                    source="manual",
        ...
        if offset % 7 == 0:
            weight = _interpolate(config.weight_start, config.weight_end, progress) + rng.uniform(-0.35, 0.35)
            systolic = _interpolate(config.bp_start, config.bp_end, progress) + rng.uniform(-3.0, 3.0)
            diastolic = max(78.0, systolic - rng.uniform(46.0, 54.0))
            readings = [
                BiomarkerReading(name="weight_kg", value=round(weight, 2), unit="kg", measured_at=_dt(current_day, 7, 30)),
                BiomarkerReading(name="systolic_bp", value=round(systolic, 2), unit="mmHg", measured_at=_dt(current_day, 7, 35)),
                BiomarkerReading(name="diastolic_bp", value=round(diastolic, 2), unit="mmHg", measured_at=_dt(current_day, 7, 35)),
            ]
            repo.save_biomarker_readings(user_id, readings)
            biomarker_count += len(readings)

        if offset % 28 == 0 or offset == days - 1:
            hba1c = _interpolate(config.hba1c_start, config.hba1c_end, progress) + rng.uniform(-0.08, 0.08)
            ldl = _interpolate(config.ldl_start, config.ldl_end, progress) + rng.uniform(-0.12, 0.12)
            readings = [
                BiomarkerReading(name="hba1c", value=round(hba1c, 2), unit="%", measured_at=_dt(current_day, 8, 30)),
                BiomarkerReading(name="ldl", value=round(ldl, 2), unit="mmol/L", measured_at=_dt(current_day, 8, 30)),
            ]
            repo.save_biomarker_readings(user_id, readings)
            biomarker_count += len(readings)
