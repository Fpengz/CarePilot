...
from dietary_guardian.features.safety.domain.alerts.models import AlertMessage, OutboxRecord
...
from dietary_guardian.features.recommendations.domain.meal_catalog_queries import DEFAULT_MEAL_CATALOG
...
class SQLiteRepository:
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_meal_nutrition_user_time ON meal_nutrition_risk_profiles(user_id, captured_at)")
    ...
    def list_meal_records(self, user_id: str) -> list[MealRecognitionRecord]:
        ...
        if legacy_records and synthesized_records:
            ...
            return merged
        if legacy_records:
            logger.debug("list_meal_records user_id=%s count=%s source=legacy", user_id, len(legacy_records))
            return legacy_records
        logger.debug("list_meal_records user_id=%s count=%s source=v2", user_id, len(synthesized_records))
    ...
    def _load_v2_meal_records(self, user_id: str) -> list[MealRecognitionRecord]:
        ...
        return [self._build_meal_record_from_event(event, profile_map.get(event.event_id)) for event in events]
    ...
    def save_meal_observation(self, observation: RawObservationBundle) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.commit()
        logger.info("save_meal_observation id=%s user_id=%s", observation.observation_id, observation.user_id)
    ...
    def save_validated_meal_event(self, event: ValidatedMealEvent) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.commit()
        logger.info("save_validated_meal_event id=%s user_id=%s", event.event_id, event.user_id)
    ...
    def save_nutrition_risk_profile(self, profile: NutritionRiskProfile) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.commit()
        logger.info("save_nutrition_risk_profile id=%s user_id=%s", profile.profile_id, profile.user_id)
    ...
    def get_nutrition_risk_profile(self, user_id: str, event_id: str) -> NutritionRiskProfile | None:
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute(
                """
                SELECT payload_json FROM meal_nutrition_risk_profiles
