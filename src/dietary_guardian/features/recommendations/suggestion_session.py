"""Session-scoped suggestion orchestration use cases.

Provides the HTTP-facing helpers ``generate_from_report``, ``list_for_session``,
and ``get_for_session``.
Kept separate from ``daily_suggestions`` to avoid a circular import between
``daily_suggestions`` → ``recommendations.use_cases`` → ``apps.api.dietary_api.deps``
→ ``dietary_guardian.agent`` → circular.
"""
...
from dietary_guardian.features.recommendations.ports import (
    BuildUserProfileFn,
    HouseholdStorePort,
