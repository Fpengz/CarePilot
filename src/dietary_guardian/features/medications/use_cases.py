...
from dietary_guardian.features.medications.intake import (
    build_plain_text_source,
    extract_upload_source,
    parse_medication_instructions,
)
from dietary_guardian.features.medications.intake.models import (
    MedicationInferenceEngineProtocol,
    MedicationIntakeDraft,
    MedicationIntakeParseResult,
    NormalizedMedicationInstruction,
)
from dietary_guardian.features.medications.domain import generate_daily_reminders
...
def _to_notification_response(item: object) -> ScheduledReminderNotificationItemResponse:
...
def _to_instruction_response(item: NormalizedMedicationInstruction) -> NormalizedMedicationInstructionResponse:
    ...
    if review_required and not allow_ambiguous:
        raise build_api_error(
            ...
            details={
                ...
                "ambiguities": [item.ambiguities for item in parse_result.instructions if item.ambiguities],
            },
        )
...
def _load_intake_draft(*, context: AppContext, user_id: str, draft_id: str) -> MedicationIntakeDraft:
...
def _run_parsed_intake(
    *,
    ...
    return MedicationIntakeResponse(
        draft_id=draft_id,
        source=MedicationIntakeSourceResponse.model_validate(parse_result.source.model_dump(mode="json")),
        normalized_instructions=[_to_instruction_response(item) for item in parse_result.instructions],
        regimens=[_to_regimen_response(item) for item in regimens],
        reminders=list(reminders),
        scheduled_notifications=[_to_notification_response(item) for item in scheduled_notifications],
    )
...
def create_regimen_for_session(
    *,
    ...
    regimen = MedicationRegimen(
        ...
        canonical_name=(payload.canonical_name or payload.medication_name.strip()).lower().replace(" ", "_"),
    ...
    *,
    ...
    try:
        ...
        trace.workflow_failed(
            ...
            details={"source_hash": source.source_hash, "source_type": source.source_type},
    ...
    *,
    ...
    try:
        ...
        trace.workflow_failed(
            ...
            error_code=str(error_code),
            message="medication intake failed",
            details={"source_hash": source.source_hash, "source_type": source.source_type, "mime_type": mime_type},
            duration_ms=(perf_counter() - started) * 1000.0,
        )
        raise
    trace.workflow_completed(
