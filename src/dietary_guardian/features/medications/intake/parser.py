...
_DOSAGE_RE = re.compile(r"(?P<name>[A-Za-z][A-Za-z0-9/-]*(?:\s+[A-Za-z][A-Za-z0-9/-]*)?)\s+(?P<dose>\d+(?:\.\d+)?\s*(?:mg|mcg|g|ml))", re.IGNORECASE)
...
def _normalize_name(raw: str) -> str:
    cleaned = re.sub(r"^(take|tab|tablet|capsule|capsules)\s+", "", raw.strip(), flags=re.IGNORECASE)
    cleaned = re.sub(r"\b\d+(?:\.\d+)?\s*(?:mg|mcg|g|ml)\b.*$", "", cleaned, flags=re.IGNORECASE)
...
def _frequency_times(text: str) -> int:
    lowered = text.lower()
    if any(token in lowered for token in ("three times a day", "three times daily", "3 times a day", "tid")):
        return 3
...
def _explicit_slots(text: str) -> list[MealSlot]:
    lowered = text.lower()
    slots: list[MealSlot] = []
    for token, slot in (("breakfast", "breakfast"), ("lunch", "lunch"), ("dinner", "dinner"), ("snack", "snack")):
        if token in lowered:
            slots.append(slot)
    return slots
...
def _normalize_instruction_dates(instruction: NormalizedMedicationInstruction, *, today: date) -> NormalizedMedicationInstruction:
    ...
    canonical = instruction.medication_name_canonical or _canonical_name(instruction.medication_name_raw)
    ...
    if instruction.timing_type == "fixed_time" and not fixed_time:
        fixed_time = "08:00"
        if "No exact administration time was provided; default morning time was used." not in ambiguities:
            ambiguities.append("No exact administration time was provided; default morning time was used.")
...
def _build_instruction(statement: str, today: date) -> NormalizedMedicationInstruction:
    ...
    if fixed_time is None and frequency_times_per_day > 1:
        ambiguities.append("No exact reminder times were provided; default meal slots were inferred.")
...
def _parse_deterministic(*, source: MedicationIntakeSource, today: date) -> MedicationIntakeParseResult:
    instructions = [
...
async def parse_medication_instructions(
    *,
    ...
    provider_name = getattr(inference_engine, "provider", None) if inference_engine is not None else None
