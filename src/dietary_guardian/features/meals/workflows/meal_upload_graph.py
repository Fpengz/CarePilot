...
from dietary_guardian.agent import arbitrate_meal_label
...
class PerceiveMeal(BaseNode[MealUploadState, MealUploadDeps, MealUploadOutput]):
    async def run(
        ...
        vision_result = normalize_vision_result(vision_result=vision_result, food_store=ctx.deps.stores.foods)
...
class ReconcileClaims(BaseNode[MealUploadState, MealUploadDeps, MealUploadOutput]):
    async def run(
        ...
        reconciled = normalize_vision_result(
            vision_result=ctx.state.vision_result.model_copy(update={"perception": reconciled_perception}),
...
class Persist(BaseNode[MealUploadState, MealUploadDeps, MealUploadOutput]):
    async def run(
        ...
        raw_observation = RawObservationBundle(
            ...
            image_quality=getattr(ctx.state.vision_result.perception, "image_quality", None)
            if ctx.state.vision_result.perception
            else None,
            confidence_score=getattr(ctx.state.vision_result.perception, "confidence_score", 0.0)
            if ctx.state.vision_result.perception
            else 0.0,
        ...
        validated_event = ValidatedMealEvent(
            ...
            confidence_summary={
                "vision_confidence": getattr(ctx.state.vision_result.perception, "confidence_score", 0.0)
                if ctx.state.vision_result.perception
                else 0.0,
            ...
            needs_manual_review=bool(enriched_event.needs_manual_review or ctx.state.unresolved_conflicts),
        )
...
async def run_meal_upload_workflow(*, deps: MealUploadDeps, state: MealUploadState) -> MealUploadOutput:
