"""Alert and reminder dispatch use cases.

Provides ``dispatch_reminder`` for direct channel delivery (v1) and
``dispatch_reminder_async`` / ``trigger_alert`` for outbox-backed v2 delivery.
These are the primary entry points for any code that needs to send a
...
from dietary_guardian.features.safety.domain.alerts import AlertDeliveryResult, AlertMessage, AlertSeverity
...
from dietary_guardian.platform.persistence.runtime_bootstrap import build_alert_repository
...
def dispatch_reminder_async(
    ...
    """Enqueue a reminder into the alert outbox and drain delivery synchronously."""
