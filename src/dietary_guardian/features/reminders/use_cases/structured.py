...
from dietary_guardian.features.reminders.domain.generation import definition_from_regimen, occurrences_for_definition
...
def _definition_summary(definition: ReminderDefinition) -> str:
    parts = [definition.medication_name, definition.dosage_text, definition.instructions_text or ""]
    return ", ".join(part for part in parts if part)


def _find_definition_for_regimen(*, context: "AppContext", user_id: str, regimen_id: str) -> ReminderDefinition | None:
    for item in context.stores.reminders.list_reminder_definitions(user_id, active_only=False):
        if item.regimen_id == regimen_id:
            return item
    return None


def _event_from_occurrence(definition: ReminderDefinition, occurrence: ReminderOccurrence) -> ReminderEvent:
    return ReminderEvent(
        ...
        status="acknowledged" if occurrence.status == "completed" else ("missed" if occurrence.status == "missed" else "sent"),
        meal_confirmation="yes" if occurrence.action == "taken" else ("no" if occurrence.action == "skipped" else "unknown"),
...
def _upsert_definition_from_regimen(
    *,
    ...
    existing = _find_definition_for_regimen(context=context, user_id=regimen.user_id, regimen_id=regimen.id)
...
def _existing_occurrence_keys(*, context: "AppContext", user_id: str) -> set[tuple[str, str]]:
    items = context.stores.reminders.list_reminder_occurrences(user_id=user_id, limit=1000)
    return {(item.reminder_definition_id, item.scheduled_for.isoformat()) for item in items}
...
def generate_structured_reminders_for_session(
    *,
    ...
    for regimen in regimens:
        definition = _upsert_definition_from_regimen(context=context, regimen=regimen)
        for occurrence in occurrences_for_definition(definition=definition, target_date=target_date, user_profile=user_profile):
            ...
            created_events.append(_event_from_occurrence(definition, saved_occurrence))

    signal_payload = {"user_id": user_profile.id, "reminder_count": len(created_events)}
    ...
    metrics = compute_mcr(context.stores.reminders.list_reminder_events(user_profile.id))
    return created_events, metrics


def list_reminder_definitions_for_user(*, context: "AppContext", user_id: str) -> list[ReminderDefinition]:
    return context.stores.reminders.list_reminder_definitions(user_id, active_only=True)
...
def list_history_occurrences_for_user(*, context: "AppContext", user_id: str) -> list[ReminderOccurrence]:
    items = context.stores.reminders.list_reminder_occurrences(user_id=user_id, limit=200)
    return [item for item in items if item.status not in {"scheduled", "queued", "processing"}]
...
def apply_occurrence_action_for_session(
    *,
    ...
    occurrence = context.stores.reminders.get_reminder_occurrence(occurrence_id)
    if occurrence is None or occurrence.user_id != user_id:
        raise KeyError("reminder occurrence not found")
    definition = context.stores.reminders.get_reminder_definition(occurrence.reminder_definition_id)
    ...
    if action == "snooze":
        ...
        updated = context.stores.reminders.update_reminder_occurrence_status(
            ...
            acted_at=acted_at,
            action=action,
            action_outcome="on_time" if acted_at <= (occurrence.scheduled_for + timedelta(minutes=occurrence.grace_window_minutes)) else "late",
        )
        ...
        if definition.regimen_id:
            context.stores.medications.save_medication_adherence_event(
                MedicationAdherenceEvent(
                    ...
                    metadata={"reminder_definition_id": definition.id, "action": action},
                )
            )
    if updated is None:
