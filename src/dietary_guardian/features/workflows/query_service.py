...
def list_workflow_runs(*, event_timeline: EventTimelineService) -> list[WorkflowRunSummary]:
    ...
    for event in events:
        ...
        if existing is None:
            ...
            continue
        latest = event.created_at if event.created_at > existing.latest_event_at else existing.latest_event_at
        by_correlation[event.correlation_id] = WorkflowRunSummary(
    ...
    return sorted(by_correlation.values(), key=lambda item: item.latest_event_at, reverse=True)
