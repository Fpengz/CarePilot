...
dietary_guardian system.  Provider credentials and network parameters are
...
class LLMSettings(BaseSettings):
    ...
    default_capability: LLMCapability = Field(
        default=LLMCapability.DIETARY_REASONING, validation_alias="LLM_DEFAULT_CAPABILITY"
    )
    capability_map: dict[LLMCapability, LLMCapabilityTarget] = Field(
        default_factory=dict, validation_alias="LLM_CAPABILITY_TARGETS"
    )
    required_provider: ModelProvider | None = Field(default=None, validation_alias="REQUIRED_PROVIDER")
    ...
    inference_wall_clock_timeout_seconds: float = Field(
        default=180.0, ge=0.1, le=3600.0,
        validation_alias="LLM_INFERENCE_WALL_CLOCK_TIMEOUT_SECONDS",
    )
    cloud_output_validation_retries: int = Field(
        default=1, ge=0, le=5, validation_alias="LLM_CLOUD_OUTPUT_VALIDATION_RETRIES"
    )
    local_output_validation_retries: int = Field(
        default=0, ge=0, le=5, validation_alias="LLM_LOCAL_OUTPUT_VALIDATION_RETRIES"
    )
    use_inference_engine_v2: bool = Field(default=True, validation_alias="LLM_USE_INFERENCE_ENGINE_V2")
