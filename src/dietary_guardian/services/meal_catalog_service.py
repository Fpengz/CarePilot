...
from dietary_guardian.models.recommendation_agent import MealCatalogItem
...
def find_catalog_item_by_title(title: str, *, locale: str = "en-SG") -> MealCatalogItem | None:
    ...
    for item in list_default_catalog(locale=locale):
        if needle == normalize_text(item.title):
            return item
    for item in list_default_catalog(locale=locale):
        if needle in normalize_text(item.title) or normalize_text(item.title) in needle:
            return item
    return None
