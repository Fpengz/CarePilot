from datetime import datetime

from pydantic import BaseModel

from dietary_guardian.logging_config import get_logger
from dietary_guardian.models.medication import ReminderEvent

logger = get_logger(__name__)

class DeliveryResult(BaseModel):
    event_id: str
    channel: str
    success: bool
    attempts: int = 1
    error: str | None = None
    delivered_at: datetime | None = None


def send_in_app(reminder_event: ReminderEvent) -> DeliveryResult:
    logger.info(
        "send_in_app event_id=%s user_id=%s medication=%s",
        reminder_event.id,
        reminder_event.user_id,
        reminder_event.medication_name,
    )
    return DeliveryResult(
        event_id=reminder_event.id,
        channel="in_app",
        success=True,
        delivered_at=datetime.utcnow(),
    )


def send_push(reminder_event: ReminderEvent, force_fail: bool = False) -> DeliveryResult:
    logger.info(
        "send_push_attempt event_id=%s user_id=%s medication=%s force_fail=%s",
        reminder_event.id,
        reminder_event.user_id,
        reminder_event.medication_name,
        force_fail,
    )
    if force_fail:
        logger.warning("send_push_failed event_id=%s reason=forced_failure", reminder_event.id)
        return DeliveryResult(
            event_id=reminder_event.id,
            channel="push",
            success=False,
            attempts=1,
            error="push delivery failed",
        )
    return DeliveryResult(
        event_id=reminder_event.id,
        channel="push",
        success=True,
        delivered_at=datetime.utcnow(),
    )


def dispatch_reminder(
    reminder_event: ReminderEvent,
    channels: list[str],
    retries: int = 2,
    force_push_fail: bool = False,
) -> list[DeliveryResult]:
    logger.info(
        "dispatch_reminder_start event_id=%s channels=%s retries=%s",
        reminder_event.id,
        channels,
        retries,
    )
    results: list[DeliveryResult] = []
    for channel in channels:
        if channel == "in_app":
            results.append(send_in_app(reminder_event))
            continue
        if channel != "push":
            logger.error(
                "dispatch_reminder_unknown_channel event_id=%s channel=%s",
                reminder_event.id,
                channel,
            )
            results.append(
                DeliveryResult(
                    event_id=reminder_event.id,
                    channel=channel,
                    success=False,
                    error="unknown channel",
                )
            )
            continue

        attempt = 0
        latest = DeliveryResult(
            event_id=reminder_event.id,
            channel="push",
            success=False,
            attempts=0,
            error="push delivery failed",
        )
        while attempt <= retries:
            attempt += 1
            latest = send_push(reminder_event, force_fail=force_push_fail)
            latest.attempts = attempt
            if latest.success:
                logger.info(
                    "dispatch_reminder_push_delivered event_id=%s attempts=%s",
                    reminder_event.id,
                    attempt,
                )
                break
        if not latest.success:
            logger.warning(
                "dispatch_reminder_push_exhausted event_id=%s attempts=%s",
                reminder_event.id,
                latest.attempts,
            )
        results.append(latest)
    logger.info("dispatch_reminder_complete event_id=%s results=%s", reminder_event.id, len(results))
    return results
