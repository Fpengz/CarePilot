...
from dietary_guardian.models.recommendation_agent import MealCatalogItem, MealSlot
...
def _from_teammate_entry(entry: dict[str, object], *, base: CanonicalFoodRecord | None = None) -> CanonicalFoodRecord:
    ...
    preparation_tags = list(dict.fromkeys([*_preparation_tags(title, str(entry.get("source") or "")), *(base.preparation_tags if base is not None else [])]))
    record = CanonicalFoodRecord(
        ...
        cuisine_tags=list(dict.fromkeys([str(entry.get("cuisine") or "").strip(), *(base.cuisine_tags if base is not None else [])])),
        ...
        nutrition=Nutrition(
            ...
            fiber_g=_number(nutrition.get("fiber_g"), base.nutrition.fiber_g if base is not None and base.nutrition.fiber_g is not None else 0),
        ),
