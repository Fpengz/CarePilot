"""Workflow coordinator for meal, alert, report, and replay orchestration."""
...
from dietary_guardian.models.workflow import WorkflowExecutionResult, WorkflowName
from dietary_guardian.services.memory_services import ClinicalSnapshotMemoryService, EventTimelineService, ProfileMemoryService
...
class WorkflowCoordinator:
    ...
    def run_meal_analysis_workflow(self, *, capture: CaptureEnvelope, vision_result: VisionResult, user_profile: UserProfile, meal_record_id: str | None = None) -> WorkflowExecutionResult:
        ...
        handoffs = [AgentHandoff(from_agent="perception_agent", to_agent="clinical_reasoning_agent", request_id=capture.request_id, correlation_id=capture.correlation_id, confidence=vision_result.primary_state.confidence_score, obligations=["evaluate_meal_against_clinical_snapshot"], payload={"dish_name": vision_result.primary_state.dish_name})]
        if vision_result.needs_manual_review:
            handoffs.append(AgentHandoff(from_agent="clinical_reasoning_agent", to_agent="notification_agent", request_id=capture.request_id, correlation_id=capture.correlation_id, confidence=vision_result.primary_state.confidence_score, obligations=["request_clarification_from_patient"], payload={"reason": "manual_review_required"}))
        self.event_timeline.append(
    ...
    def run_alert_workflow(self, *, user_profile: UserProfile, alert_type: str, severity: AlertSeverity, message: str, destinations: list[str], request_id: str | None = None, correlation_id: str | None = None, account_role: str = "member", scopes: list[str] | None = None, environment: str = "dev") -> WorkflowExecutionResult:
        ...
        handoffs = [AgentHandoff(from_agent="coordinator", to_agent="notification_agent", request_id=issued_request_id, correlation_id=issued_correlation_id, confidence=1.0 if tool_result.success else 0.0, obligations=["deliver_alert_via_channels"], payload={"alert_type": alert_type, "destinations": destinations})]
