...
class WorkflowCoordinator:
    ...
    def run_meal_analysis_workflow(
        self,
        *,
        ...
        handoffs = [
            AgentHandoff(
                from_agent="perception_agent",
                to_agent="clinical_reasoning_agent",
                request_id=capture.request_id,
                correlation_id=capture.correlation_id,
                confidence=vision_result.primary_state.confidence_score,
                obligations=["evaluate_meal_against_clinical_snapshot"],
        ...
        if vision_result.needs_manual_review:
            handoffs.append(
                AgentHandoff(
                    from_agent="clinical_reasoning_agent",
        ...
        return WorkflowExecutionResult(
            ...
            handoffs=handoffs,
            timeline_events=self.event_timeline.list(correlation_id=capture.correlation_id),
        )
    ...
    def replay_workflow(self, correlation_id: str) -> WorkflowExecutionResult:
        ...
        logger.info(
            "workflow_replay correlation_id=%s events=%s side_effects=false",
            correlation_id,
            len(events),
        )
        return WorkflowExecutionResult(
