...
def _build_macro_chart(
    ...
    totals: dict[datetime, dict[str, float]] = defaultdict(lambda: {"protein": 0.0, "carbs": 0.0, "fat": 0.0, "calories": 0.0})
    for item in profiles:
...
def _build_meal_timing_chart(meals: list[ValidatedMealEvent]) -> DashboardMealTimingChartResponse:
    counts = [0] * 24
    for meal in meals:
...
def _build_summary(
    *,
    ...
    calorie_score = max(0.0, 100.0 - (abs(avg_daily_calories - calorie_target) / calorie_target * 100.0))
    previous_calorie_score = max(0.0, 100.0 - (abs(previous_daily_calories - calorie_target) / calorie_target * 100.0))
    ...
    calorie_variance = abs(avg_daily_calories - calorie_target) / calorie_target if calorie_target else 0.0
    ...
    return DashboardSummaryResponse(
        ...
        glycemic_risk=DashboardSummaryMetricResponse(
            ...
            status=risk_status,
            detail=f"Latest HbA1c {hba1c:.1f}." if hba1c is not None else "No HbA1c reading available yet.",
        ),
...
def _build_insights(
    *,
    ...
    average_calories = _average_daily_calories(current_profiles, 1 if not current_profiles else len({item.captured_at.date() for item in current_profiles}))
    if summary.glycemic_risk.status in {"watch", "elevated"}:
        recommendations.append("Shift one high-carb dinner this week toward a higher-protein, higher-fiber plate.")
        key_drivers.append("Repeated high-risk meal tags are pushing glycemic risk above the target band.")
    if summary.adherence_score.value < 90:
        recommendations.append("Tighten reminder timing around the missed dose window and keep one backup notification channel active.")
        key_drivers.append("Medication adherence is the fastest lever to improve short-term stability.")
    if average_calories > calorie_target * 1.08:
        recommendations.append("Trim the highest-calorie meal by 10-15% rather than cutting across the whole day.")
        key_drivers.append("Average calorie intake is running above the configured target.")
    if not recommendations:
        recommendations.append("Keep the current cadence and review deeper analytics only if the trend turns for two consecutive weeks.")
        key_drivers.append("Nutrition and adherence are both inside the steady zone.")
    return DashboardInsightsResponse(recommendations=recommendations[:3], key_drivers=key_drivers[:3])
...
def get_dashboard_overview(
    *,
    ...
    all_adherence = context.stores.medications.list_medication_adherence_events(user_id=user_id, limit=1000)
    ...
    current_adherence = _filter_events(all_adherence, start=current.start, end=current.end, accessor="scheduled_at")
    previous_adherence = _filter_events(all_adherence, start=comparison.start, end=comparison.end, accessor="scheduled_at")
    current_reminders = _filter_events(all_reminders, start=current.start, end=current.end, accessor="scheduled_at")
    current_meals = _filter_events(all_meals, start=current.start, end=current.end, accessor="captured_at")
    ...
    charts = DashboardChartsResponse(
        calories=_build_calorie_chart(windows, current.bucket, current_profiles, calorie_target=calorie_target),
    ...
    return DashboardOverviewResponse(
        ...
        charts=charts,
        insights=_build_insights(summary=summary, calorie_target=calorie_target, current_profiles=current_profiles),
    )
