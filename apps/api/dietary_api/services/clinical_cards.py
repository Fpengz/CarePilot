...
def generate_clinical_card_for_session(
    *,
    ...
    biomarker_readings = context.repository.list_biomarker_readings(user_id)
    symptom_items = context.repository.list_symptom_checkins(user_id=user_id, limit=500)
    adherence_items = context.repository.list_medication_adherence_events(user_id=user_id)
    ...
    saved = context.repository.save_clinical_card(card)
...
def list_clinical_cards_for_session(
    *,
    ...
    items = context.repository.list_clinical_cards(user_id=user_id, limit=limit)
...
def get_clinical_card_for_session(
    *,
    ...
    item = context.repository.get_clinical_card(user_id=user_id, card_id=card_id)
    if item is None:
