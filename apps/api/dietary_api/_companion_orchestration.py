"""API-layer orchestration for companion endpoints.

Bridges the API AppContext with the companion application use cases.
Contains context-building helpers and the four service functions that route
handlers call directly.  This lives in the API layer because it accepts
AppContext and assembles HTTP response schemas.
"""
...
from dietary_guardian.application.evidence import EvidenceRetrievalPort
...
_EVIDENCE_RETRIEVER: EvidenceRetrievalPort = StaticEvidenceRetriever()
...
# ---------------------------------------------------------------------------
# API-layer service functions (called by routers)
# ---------------------------------------------------------------------------
