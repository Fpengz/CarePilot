...
FOOD_COLLECTION = "food_knowledge"
...
class FoodHybridSearch:
    """Food 领域检索 Agent（向量召回 + 关键词 rerank）"""
    ...
    def __init__(
        ...
        try:
            ...
            print(f"[警告] 向量检索初始化失败，将仅使用关键词检索: {exc}")

    def search(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        """执行 food 检索：向量召回 + 关键词 rerank"""
        query = query.strip()
        if not query:
            return []
    ...
    def _vector_search(self, query: str, disease: str | None, top_k: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            print(f"[警告] 向量检索执行失败，回退关键词检索: {exc}")
            return []
    ...
    def _keyword_rerank(
        ...
        for idx, item in enumerate(candidates):
            ...
            name_boost = 0.0
            for key in ("food_name", "food_name_cn", "drink_name"):
            ...
            print(f"  [Debug] Candidate {idx+1}: overlap={overlap}, coverage={coverage_score:.3f}, phrase={phrase_score:.3f}, name_boost={name_boost:.3f}, keyword_score={keyword_score:.3f}")
...
if __name__ == "__main__":
    agent = FoodHybridSearch()
    print("\n=== Food Vector Retrieval + Keyword Rerank Agent ===")
    print("输入问题进行检索，Ctrl+C 退出")
    print("示例: 糖尿病可以喝Kopi吗")
    ...
    try:
        while True:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                print(
                    f"\n[{idx}] rerank={row.get('rerank_score', 0.0):.3f} | "
                    f"keyword={row.get('keyword_score', 0.0):.3f} | "
                    f"vector={row['vector_score']:.3f}"
                )
                print(
                    f"chunk={meta.get('chunk_type', '')} | "
                    f"food={meta.get('food_name', meta.get('drink_name', ''))} | "
                    f"disease={meta.get('disease', '-') }"
                )
