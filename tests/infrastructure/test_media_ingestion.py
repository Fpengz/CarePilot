...
def test_duplicate_capture_suppression_uses_content_hash_and_window() -> None:
    ...
    first = should_suppress_duplicate_capture(
        session_state, envelope, window_seconds=30
    )
    second = should_suppress_duplicate_capture(
        session_state, envelope, window_seconds=30
    )
...
def test_duplicate_capture_suppression_expires_after_window() -> None:
    ...
    session_state = {
        "capture_dedupe:default": {
            envelope.content_sha256: datetime(
                2020, 1, 1, tzinfo=timezone.utc
            ).isoformat(),
        }
    }

    suppress = should_suppress_duplicate_capture(
        session_state, envelope, window_seconds=1
    )
