...
def test_require_session_accepts_live_session_when_duplicate_cookie_contains_expired_first() -> (
    None
):
    app = create_app()
    ...
    old_cookie, old_session_id = _login_and_get_cookie(
        client, email="member@example.com", password="member-pass"
    )
    assert old_cookie
...
def test_require_session_accepts_live_session_when_duplicate_cookie_contains_invalid_first() -> (
    None
):
    app = create_app()
    client = TestClient(app)
...
def test_require_session_returns_session_expired_when_all_signed_candidates_are_dead() -> None:
    ...
    cookie_b, session_id_b = _login_and_get_cookie(
        client, email="member@example.com", password="member-pass"
    )
